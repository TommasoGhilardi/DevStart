...
Raw_data = pd.read_csv('DATA\\RAW\\Adult1.csv')
```
...
# Scatterplot
sns.scatterplot(data=SaccadicLatency, x="Event_trial", y="Latency", hue='Event')
...
Again, we can now visualise the data. For example, we can plot whether looking time to rewarding vs. non-rewarding stimuli:
...
# Scatterplot
sns.scatterplot(data=LookingTime, x="Event", y="dur", hue='Event')
