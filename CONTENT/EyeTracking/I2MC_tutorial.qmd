...
Before doing anything else I would suggest creating a folder where we will save the output of I2MC. We will create a folder called *i2mc_output*. Using the *os* library we make sure the output folder dosen't exist (`os.path.isdir(output_fodler)`) and then we create it (`os.mkdir(output_fodler)`)
