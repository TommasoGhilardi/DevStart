...
# This section fits two models to the simulated data. The first is a simple linear model (LM) that
# includes the fixed effects of the categorical and continuous variables, as well as their interaction.
# The second is a mixed-effects model (LMM) that also includes random intercepts and slopes for subjects.

# Linear Model (LM)
mod_lm <- lm(dependent_variable ~  categorical_condition * trial_number, data = simulated_data)
summary(mod_lm)


# Mixed-Effects Model (LMM)
mod_mixed <- lmer(dependent_variable ~  categorical_condition * trial_number + 
...
Est <- estimate_expectation(mod_mixed, get_datagrid(mod_mixed, by = c('categorical_condition','trial_number')))
ggplot(Est, aes(x = trial_number, y = Predicted, color = categorical_condition, fill = categorical_condition)) +
