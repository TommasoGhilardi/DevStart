...
df$Event_trial_scaled = scale(df$Event_trial) # standardize the continuous variable
```
...
#| message: false
#| warning: false
estimate_slopes(mod_gamma, trend = 'Event', by = 'Event_trial_scaled')
```
...
ggplot(is_pred_random, aes(x= Event_trial_scaled, y= Predicted, color= Id, shape = Event))+
