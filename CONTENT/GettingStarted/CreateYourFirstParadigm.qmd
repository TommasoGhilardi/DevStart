...
Python 3.4+ includes the `pathlib` module which provides a more intuitive, object-oriented approach to working with file paths that works across all operating systems:
...
Using `pathlib` offers several advantages: cross-platform compatibility with automatic correct separators, intuitive path joining with the `/` operator, built-in methods for file operations (like `.exists()` and `.glob()`), and no need to handle backslash escaping.

For this tutorial series, we'll use `pathlib` for better cross-platform compatibility, but it's good to understand both approaches!
::::
